class AmplifiedControllerSoftware {
  constructor (integers) {
    ...
    this._initializeIntegers(integers)
  }

  _initializeIntegers (integers) {
    const arr = new Array(2 ** 24).fill('0')
  ...
  handleInstruction () {
    switch (this.instructionOpcode) {
      ...
      case '9': return this.relativeBaseOffset()
    }
  }

  relativeBaseOffset () {
    const mode = this.parameterModes[this.parameterModes.length - 1]
    const parameter = this.integers[this.integersIndex + 1]
    const value = this.getActualValue(mode, parameter)
  ...
  handleBinaryOperation (operation) {
    ...
    const mode3 = this.parameterModes[this.parameterModes.length - 3]
    ...
    this.setActualValue(mode3, parameter3, result.toString())
    
    return this.integersIndex + 4
  }

  getActualValue (mode = '0', parameter) {
    const intParameter = Number.parseInt(parameter)

    switch (mode) {
      // position mode
      case '0': return Number.parseInt(this.integers[intParameter])
      // immediate mode
      case '1': return Number.parseInt(intParameter)
       // relative mode
      case '2': return Number.parseInt([this.integers[this.relativeBase + intParameter]])
    }
  }

  setActualValue (mode, parameter, value) {
    const intParameter = Number.parseInt(parameter)

    switch (mode) {
      // relative mode
      case '2': return this.integers[this.relativeBase + intParameter] = value
      // position mode
      default: return this.integers[intParameter] = value
    }
  }
  ...
  input () {
    ...
    const mode = this.parameterModes[this.parameterModes.length - 1]
    const parameter = this.integers[this.integersIndex + 1]
    this.setActualValue(mode, parameter, inputValue)
  ...
  compare (validateCondition) {
    ...
    if (validateCondition(value1, value2)) {
      this.setActualValue(mode3, parameter3, '1')
    } else {
      this.setActualValue(mode3, parameter3, '0')
    }
